...
public class StreamApiExample {
	...
	public static void main(String[] args) {
		...
		Order order1 = new Order(1L, "Delivered", LocalDate.of(2021, 2, 15), LocalDate.of(2021, 2, 18), products1,
				customer1);
		Order order2 = new Order(2L, "Pending", LocalDate.of(2021, 3, 10), LocalDate.of(2021, 3, 12), products2,
				customer2);
		Order order3 = new Order(3L, "Delivered", LocalDate.of(2021, 4, 20), LocalDate.of(2021, 4, 22), products3,
				customer3);
		...
		List<Product> discountedToys = productList.stream().filter(p -> p.getCategory().equals("Toys")).map(p -> {
		...
		// Task 4: Obtain a list of products ordered by tier 2 customers between
		// 01-Feb-2021 and 01-Apr-2021
		List<Product> tier2OrderedProducts = orderList.stream().filter(order -> order.getCustomer().getTier() == 2)
		...
		Optional<Product> cheapestBook = productList.stream().filter(p -> p.getCategory().equals("Books"))
		...
		List<Order> recentOrders = orderList.stream().sorted((o1, o2) -> o2.getOrderDate().compareTo(o1.getOrderDate())) // Descending
																															// order
		...
		// Task 8: Get statistics (sum, average, max, min, count) for all products in
		// 'Books' category
		DoubleSummaryStatistics stats = productList.stream().filter(p -> p.getCategory().equals("Books"))
		...
		Optional<Product> mostExpensiveBook = productList.stream().filter(p -> p.getCategory().equals("Books"))
